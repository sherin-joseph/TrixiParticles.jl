...
function pressure_function(pos, t)
    ...
    return -U^2 * exp(2 * b * t) * (cos(4pi * x) + cos(4pi * y) + cos(4pi * z)) / 4
end
...
function velocity_function(pos, t)
    ...
    vel = U * exp(b * t) * [sin(2pi * x) * cos(2pi * y) * cos(2pi * z),
                            -cos(2pi * x) * sin(2pi * y) * cos(2pi * z),
                            0.0]
...
n_particles_xyz = round(Int, box_length / particle_spacing)
...
fluid = RectangularShape(particle_spacing, (n_particles_xyz, n_particles_xyz, n_particles_xyz), (0.0, 0.0, 0.0),
...
# ==========================================================================================
# ==== Simulation
periodic_box = PeriodicBox(min_corner=[0.0, 0.0, 0.0], max_corner=[box_length, box_length, box_length])
