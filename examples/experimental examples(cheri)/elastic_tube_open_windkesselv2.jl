...
# ------------------------------------------------------------------------------------------
# NEW: remove any SOLID particles inside the OPEN BOUNDARY slab (prevents overlap)
# ------------------------------------------------------------------------------------------
function remove_solid_in_open_boundary_regions(
    solid_ic, inlet_face, outlet_face;
    ...
    function carve!(face, R_eff)
        t1, t2, _, _ = face_basis(face)
        c = face_center(face)
        n = normalize(cross(t1, t2))                 # face normal (either direction is fine; we use abs distance)
        ...
        for i in 1:size(X,2)
            ...
            d = dot(ξ, n)                            # signed distance to plane
            if abs(d) <= slab
                r = sqrt((dot(ξ, t1))^2 + (dot(ξ, t2))^2)  # in-plane radius around face center
                if r <= r_cut
                    keep[i] = false
                end
            end
    ...
    carve!(inlet_face,  R_in_eff)
    carve!(outlet_face, R_out_eff)
...
# ------------------------------------------------------------------------------------------
# NEW CALL: ensure NO solid particles exist inside open-boundary regions
# ------------------------------------------------------------------------------------------
